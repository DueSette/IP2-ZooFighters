        ...
        if (gameManager != null)
        {
            ...
            if (gameManager.GetGameState() == GameManagerScript.GameState.charSelect)
            {
                ...
                //ACTIVATING THE SELECTOR, SELECTING AND DESELECTING CHARACTER
                    ...
                    {
                        if (selectors[0].GetComponent<SelectorBehaviour>().chosenCharacter == null && !aSuspend)
                        {
                            selectors[0].GetComponent<SelectorBehaviour>().CharSelect();

                            if (suspendHandle == null)
                            {
                                suspendHandle = StartCoroutine(SuspendControl());
                            }
                            else
                            {
                                StopCoroutine(suspendHandle);
                                suspendHandle = StartCoroutine(SuspendControl());
                            }
                        }
                    }
                ...
                #region JOY 2
                    ...
                    {
                        if (selectors[1].GetComponent<SelectorBehaviour>().chosenCharacter == null && !aSuspend)
                        {
